...
def sft_post():
    ...
    is_save = question_data.get('is_save', 0)
    ...
    if streaming == 0:
        ...
        for i, j in enumerate(cosyvoice.inference_sft(text, speaker, stream=False, speed=speed,
                                                      new_dropdown=speaker if speaker not in default_voices else "无",
                                                      is_save=flag)):
    ...
    # 流式模式
        ...
        def generate():
            ...
            for i, j in enumerate(cosyvoice.inference_sft(text, speaker, stream=True, speed=speed,
                ...
                tts_speeches.append(j['tts_speech'])
                audio_data = torch.concat(tts_speeches, dim=1)
                torchaudio.save(buffer, audio_data, sample_rate, format="ogg")
                buffer.seek(0)
...
def save_voice():
    ...
    audio_data = torch.concat(tts_speeches, dim=1)
    torchaudio.save('zero_shot.wav', audio_data, sample_rate, format="wav")
